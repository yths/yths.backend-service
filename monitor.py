...
def job_audio(r):
    p=subprocess.Popen("pactl get-sink-mute @DEFAULT_SINK@", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout, _ = p.communicate()
    muted = stdout.decode("utf-8").strip().split(" ")[-1] == "yes"
    p=subprocess.Popen("pactl get-sink-volume @DEFAULT_SINK@", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout, _ = p.communicate()
    volume = int(stdout.decode("utf-8").strip().split("/")[1].strip().replace("%", ""))
